var CodeMirror = (function() {
        ...
        function cf(c6) {
            ...
            function db(dx, ds, dt) {
                ...
                for (var dn = c7 ? 0 : dw.length - 2, dq = c7 ? dw.length : -2; dn != dq; dn += 2 * dj) {
                    ...
                    for (var dm = c7 ? 0 : dv.length - 1, dl = c7 ? dv.length : -1; dm != dl; dm += dj,
                    dr += dj) {
    ...
    i.prototype = {
        ...
        maybeSpill: function() {
            ...
            do {
                ...
                ap.parent = aq.parent
            } while (aq.children.length > 10);aq.parent.maybeSpill()
        },
    ...
    function Y(au, ar) {
        ...
        outer: do {
            ...
            return ap
        } while (!au.lines);for (var aq = 0, at = au.lines.length; aq < at; ++aq) {
            var aw = au.lines[aq]
              , av = aw.height;
            if (ar < av) {
                break
    ...
    function g(an, au) {
        ...
        outer: do {
            ...
            return ap
        } while (!an.lines);for (var ao = 0; ao < au; ++ao) {
    ...
    function X(aq, ap) {
        ...
        for (var ao = aq.length, an = ap.length; ao >= 0 && an >= 0; --ao,
        --an) {
...
CodeMirror.defineMode("xml", function(z, k) {
    ...
    return {
        ...
        compareStates: function(E, C) {
            ...
            for (var D = E.context, B = C.context; ; D = D.prev,
            B = B.prev) {
                if (!D || !B) {
                    return D == B
                }
...
Ext.define("Docs.view.cls.Toolbar", {
    ...
    initComponent: function() {
        ...
        var g = this;
        g.items = g.items.concat([{
            xtype: "tbfill"
        }, g.filterField = Ext.widget("triggerfield", {
            triggerCls: "reset",
            cls: "member-filter",
            hideTrigger: true,
            ...
            listeners: {
                buffer: 256,
                change: function() {
                    var v = g.filterField.getValue();
                    g.fireEvent("filter", v, g.getShowFlags());
                    g.filterField.setHideTrigger(v.length === 0);
                },
...
var q = null;
    ...
    function d(F) {
        ...
        function A(J) {
            for (var M = J.substring(1, J.length - 1).match(/\\u[\dA-Fa-f]{4}|\\x[\dA-Fa-f]{2}|\\[0-3][0-7]{0,2}|\\[0-7]{1,2}|\\[\S\s]|[^\\]/g), J = [], I = [], O = M[0] === "^", P = O ? 1 : 0, L = M.length; P < L; ++P) {
                ...
                if (/\\[bdsw]/i.test(K)) {
                    ...
                    var K = w(K), N;
                    P + 2 < L && "-" === M[P + 1] ? (N = w(M[P + 2]),
                    P += 2) : N = K;
                    I.push([K, N]);
                    N < 65 || K > 122 || (N < 65 || K > 90 || I.push([Math.max(65, K) | 32, Math.min(N, 90) | 32]),
                    N < 97 || K > 122 || I.push([Math.max(97, K) & -33, Math.min(N, 122) & -33]))
                }
            }
            I.sort(function(Q, R) {
            ...
            for (P = 0; P < I.length; ++P) {
                L = I[P],
                L[0] <= K[1] + 1 ? K[1] = Math.max(K[1], L[1]) : M.push(K = L)
            }
            I = ["["];
            O && I.push("^");
            I.push.apply(I, J);
        ...
        function E(J) {
            ...
            for (L = O = 0; O < I; ++O) {
                K = M[O],
                K === "(" ? (++L,
                N[L] === void 0 && (M[O] = "(?:")) : "\\" === K.charAt(0) && (K = +K.substring(1)) && K <= L && (M[O] = "\\" + N[L])
            }
            for (L = O = 0; O < I; ++O) {
                "^" === M[O] && "^" !== M[O + 1] && (M[O] = "")
            }
            if (J.ignoreCase && H) {
                for (O = 0; O < I; ++O) {
                    K = M[O],
                    J = K.charAt(0),
                    K.length >= 2 && J === "[" ? M[O] = A(K) : J !== "\\" && (M[O] = K.replace(/[A-Za-z]/g, function(P) {
                        P = P.charCodeAt(0);
    ...
    function r(H, x) {
        ...
        for (var w = Math.max(0, x - 1 | 0) || 0, D = 0, G = F.length; D < G; ++D) {
            A = F[D],
            A.className = "L" + (D + w) % 10,
            A.firstChild || A.appendChild(J.createTextNode("\xa0")),
            u.appendChild(A)
        }
        H.appendChild(u)
    }
        ...
        try {
            ...
            for (U.length = H; Q < I; ) {
                ...
                if (P.nodeType !== 1 && (O = F.substring(T, W))) {
                    ...
                    B.replaceChild(D, P);
                    D.appendChild(P);
                    T < J && (M[Q + 1] = P = E.createTextNode(F.substring(W, J)),
                    B.insertBefore(P, D.nextSibling))
                }
                T = W;
                T >= J && (Q += 2);
                T >= V && (X += 2)
        ...
        function v() {
            for (var L = window.PR_SHOULD_USE_CONTINUATION ? w.now() + 250 : Infinity; u < A.length && w.now() < L; u++) {
                ...
                if (I.indexOf("prettyprint") >= 0) {
                    ...
                    H || ((H = (H = O.className.match(/\blinenums\b(?::(\d+))?/)) ? H[1] && H[1].length ? +H[1] : !0 : !1) && r(O, H),
                    D = {
                        g: I,
                        h: O,
                        i: H
